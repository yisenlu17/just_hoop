...
import { formatTime, MATCH_TYPE_LABEL } from "@/lib/domain";
...
export async function PlayerDashboard() {
  ...
  const recommended = matches.filter((match) => match.id !== nextMatch?.id).slice(0, 3);
        ...
        <section className="grid gap-5">
          ...
          <Panel className="p-4 sm:p-5">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-black text-white">推荐房间</h2>
              <ButtonLink href="/player/matches" tone="dark" className="min-h-9 px-3 py-1.5 text-xs">
                全部比赛
              </ButtonLink>
            </div>
